...
Numeric claims hard-gate: every load-bearing figure must be supportable by 1-2 contiguous sentences in the cited source that contain those numbers; do not invent mechanisms or before/after stories absent from that span.         "GitHub Awesome-lists are Band C. arXiv preprints are NOT peer-reviewed (label preprint, Band B). Predatory venues (e.g. IJSR) are Band C / unreliable — never Band A. arXiv+OpenReview of the same paper = one work. Keep numeric scope: do not promote a small-model/HAR/CPU measurement into a universal 70B LLM fact. "
